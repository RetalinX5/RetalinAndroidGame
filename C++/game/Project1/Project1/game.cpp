...
float player_half_size_x = 2.5, player_half_size_y = 15;
float ball_p_x, ball_p_y, ball_dp_x = 130, ball_dp_y, ball_half_size = 1.5;
...
simulate_game(Input* input, float dt) {
	...
	if (current_gamemode == GM_GAMEPLAY) {
		...
		if (!enemy_is_ai) {
			if (is_down(BUTTON_UP)) player_1_ddp += 3000;
			if (is_down(BUTTON_DOWN)) player_1_ddp -= 3000;
		}
		else {
		...
		draw_number(player_1_score, -20, 40, 1.f, 0x0000FF);
		draw_number(player_2_score, 20, 40, 1.f, 0xff0000);
		...
		draw_text("RETALIN GAME ", -73, 40, 2, 0xffffff);
		draw_text(" YOUTUBE ", -73, 22, .75, 0xffffff);
		draw_text("MADARA", -73, 15, 1.22, 0xffffff);
